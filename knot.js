+function() {
  ...
  request.onload = function() {
    ...
    init(values.slice(1, parseInt(values[0]) + 1).map(parseFloat));
  }
   // knot.txt from http://www.mathematik.uni-marburg.de/~thormae/lectures/graphics1/code/WebGLShaderLightMat/ShaderLightMat.html
   // or http://www.mathematik.uni-marburg.de/~thormae/lectures/graphics1/code/WebGLShaderLightMat/knot.txt
  request.open('get', 'knot.txt', true);
  request.send();
  ...
  function init(vertices) {
    ...
    var colors = get_colors(vertices.length / 8 * 3);
    ...
    console.log('r', r, 'g', g, original_length / 8)
    ...
    gl.vertexAttribPointer(posX, 1, gl.FLOAT, false, 4 * 8, 0);
    gl.vertexAttribPointer(posY, 1, gl.FLOAT, false, 4 * 8, 4);
    gl.vertexAttribPointer(posZ, 1, gl.FLOAT, false, 4 * 8, 8);
    gl.vertexAttribPointer(r, 1, gl.FLOAT, false, 0, 4 * original_length);
    gl.vertexAttribPointer(g, 1, gl.FLOAT, false, 0, 4 * original_length + 4 * original_length / 8);
    gl.vertexAttribPointer(b, 1, gl.FLOAT, false, 0, 4 * original_length + 4 * original_length / 8 + 4 * original_length / 8);
